...
public class RegisterCommandValidation : AbstractValidator<RegisterCommand>
{
    public RegisterCommandValidation()
    {
        ...
        RuleFor(x => x.ConfirmPassword).Cascade(CascadeMode.Stop).Equal(x => x.Password).NotEmpty()
            .MinimumLength(8).MaximumLength(40).Matches(Regexes.PasswordRegex);
        RuleFor(x => x.IconType).Cascade(CascadeMode.Stop).NotEmpty();
    }
}